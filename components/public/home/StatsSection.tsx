        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
          {stats.map((stat, index) => (
            ...
            >
              ...
              <div className="mt-2 font-medium text-zinc-900 dark:text-white">{t(`pages.home.statsSection.stats.${stat.label}.label`)}</div>
              <div className="text-sm text-zinc-600 dark:text-zinc-400">{t(`pages.home.statsSection.stats.${stat.label}.description`)}</div>
            </div>
          ))}
        </div>
      </div>
