...
const ResetPasswordForm = ({ token }: ResetPasswordFormProps) => {
  ...
  const {
    ...
    setErrorsState,
    touched,
  } = useFormValidation(validateResetPasswordFormData, locale as LocaleEnumType);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.status === 400 && result.validationErrors) {
        ...
        addToast({
          color: "danger",
          title: t("pages.auth.resetPassword.metadata.title"),
        ...
        addToast({
          color: "success",
          title: t("pages.auth.resetPassword.metadata.title"),
          description: t("common.toast.success.passwordReset"),
          timeout: 5000,
        });
      ...
      addToast({
        color: "danger",
        title: t("pages.auth.resetPassword.metadata.title"),
        description: error?.message || t("common.toast.error.resetPasswordFailed"),
      });
    } finally {
      setIsPending(false);
    }
  ...
  const ErrorMessages = ({ field }: { field: keyof ResetPasswordFormData }) => {
          ...
          description={
            <div className="flex flex-col gap-2">
              <p className="font-medium">
                {t("pages.auth.resetPassword.success.title")}
              </p>
              <p className="text-sm">
                {t("pages.auth.resetPassword.success.description")}
              </p>
            </div>
          }
        />
  ...
  return (
    <form className="flex flex-col w-full gap-4" onSubmit={handleSubmit}>
      <Alert
        ...
        description={
          <p className="text-sm">
            {t("pages.auth.resetPassword.instructions")}
          </p>
        }
      />
      ...
      <div className="flex items-center justify-between">
        <div className="text-xs text-default-600">
          <span>{t("pages.auth.resetPassword.remember_password")}</span>
          <Link
            hrefLang={locale}
            href={"/auth/login"}
