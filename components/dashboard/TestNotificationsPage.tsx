...
export default function TestNotificationsPage() {
  ...
  const loadUsersFromAPI = async () => {
    ...
    try {
      // Fetch all users
      const usersResponse = await fetch('/api/users?type=all');
      ...
      // Fetch admin users
      const adminsResponse = await fetch('/api/users?type=admins');
                      ...
                      {filteredUsers.map((user) => (
                        <div 
                          ...
                          className={`flex items-center gap-3 p-3 rounded-lg transition-colors ${
                            selectedUserIds instanceof Set && selectedUserIds.has(user.id)
